...
fly.info = read.csv("D:/Behavioral_project/Behavior Experiment Data/Sorted_data_experimenter/fly_info_CS_controls.csv",header=T,stringsAsFactors=F)
...
for(ind in 1:nrow(fly.info)){        
  ...
  for(session in query.sessions){
    ...
    if (dim(read.csv(input.file,stringsAsFactors = F))[2]==2){#
      laser.status <- read.csv(input.file,stringsAsFactors = F)[,2]#
    }#
    ...
    if(((fly.info$experimenter[ind] == "JG")&(fly.info$Fly[ind]<=72))
       ...
       |((fly.info$experimenter[ind]=="JE")&(fly.info$Fly[ind]<=71))
    ...
    if(session == "E1"){
      ...
      cat(fly.info$Fly[ind],fly.info$Category[ind],input.file,"\n",sep="\t")
    }
    
    fly.pos.dat = data.frame(fly.pos, laser.status)
    colnames(fly.pos.dat) = c(paste0("fly_pos;framerate=",framerate), "laser_status")#
    
    dir.create(paste0("data/",fly.info$experimenter[ind],"/mutants/"), showWarnings = FALSE, recursive = TRUE)
