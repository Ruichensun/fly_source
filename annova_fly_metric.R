...
metrices <- read.table("metrics/list_metrices.csv",
                       stringsAsFactors=F,
                       sep="\t")[,1]
pdf("frequency_080317_WT_Mar2017-present_filter1_Normalized_Batch1_3.pdf",onefile=T,width=10)
for(metric.ind in 1:length(metrices)){
  ...
  df = data.frame(metric=as.numeric(y$value),
  ...
  for(category in c("T","R","N")){
    ...
    print(p)
    hist(log(abs(df$metric[ind])),breaks=50,
         main=paste0(category," ",metrices[metric.ind],"\np = ",p),
         xlab="value",ylab="freq")
    
    for(session in sessions){
      if(sum(df$session==session & ind) < 3){plot(c(0,1),c(0,1),type='n');next}
      p = shapiro.test(log(abs(df$metric[df$session==session & ind])+exp(-10)))$p.value
      hist(log(abs(df$metric[df$session==session & ind])),breaks=50,
  ...
  model = lme(metric ~ 1+session+category+session:category, 
              random = ~1|fly.id,
              data=df,
              method="REML")
  print(anova(model))
...
pdf("frequency_session_only_log_transformed_080317_WT_Mar2017-present_filter1_Normalized_Batch1_3.pdf",onefile=T,width=10)
for(metric.ind in 1:length(metrices)){
  input.file = paste0("metrics/metric_",metric.ind,".csv")
  if(!file.exists(input.file)){next}
