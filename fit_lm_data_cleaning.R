...
pass_fly_QC <- function(input_file,
                        framerate = 50,
                        speed_max_thres = 50, #changed to 50 on April 2018
...
excl.fly.WT = data.frame(
  cbind(
        ...
        c("RS","JD",rep("ES",400),rep("RS",100),rep("JD",40)),#added JD's 40 flies on 5/19/18, maybe removed later
...
#Mutant data
  ...
  print(excl.fly[ind,])
  print(fly.info[which(fly.info$Genotype == excl.fly[ind,3] &
                fly.info$experimenter == excl.fly[ind,2] &
...
for(genotype in unique(fly.info$Genotype)){
  ...
  ind.filter =  rank_fms <= length(fms) * 1 & #changed from 1 to 0.95 on Apr 13, 2018; Back to 1 on Apr 23, 2018
