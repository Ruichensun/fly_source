...
#Step 2: Run the function from Step 1 on all files. 
  ...
  for(ind in 1:nrow(fly.info.include)){
    ...
    for(ind.session in 1:length(query.sessions)){
      if ((fly.info.include[ind,]$Genotype == "WT")|(fly.info.include[ind,]$Genotype == "CS")) {
        # print("Yes")}else{print("ERROR")}
        input.file <- list.files(path = paste0("data/",
                                               fly.info.include$experimenter[ind],
                                               "/CS/"),                             
                                 pattern = paste0("ProcessedData_Fly",fly.info.include$Fly[ind],
        ...
        input.file <- list.files(path = paste0("data/",
                                 ...
                                 pattern = paste0("ProcessedData_Fly",fly.info.include$Fly[ind],
                                                  "_",query.sessions[ind.session],
                                                  "_",fly.info.include$Genotype[ind],
                                                  ".csv"),
                                 full.names=T
                                 )
      }
...
###
  ...
  for(ind in 1:length(metrics)){
    ...
    colnames(metric.df) = c("Value","Fly","Category","Gender","Genotype","Experimenter","Age","Setup","Session")
...
##### Classifying flies into long-heat group and short-heat group by the duration of heat they received -- Using the "Total laser exposure in Seconds" Metric
  ...
  metric.df.WT = metric.df[(metric.df$Genotype=="WT")&!(metric.df$Experimenter=="SW"),] # This can be changed for different genotypes
  ...
  # 
  for (i in 1:dim(metric.df.WT.E1T1)[1]){
    AllT1 = c(AllT1,sum(metric.df.WT[(metric.df.WT$Fly==metric.df.WT.E1T1[i,]$Fly)&(metric.df.WT$Experimenter==metric.df.WT.E1T1[i,]$Experimenter)&!is.na(metric.df.WT$Value),]$Value[1:5]))
  }
  ...
  for (i in 1:dim(metric.df.WT.E1R1)[1]){
    AllR1 = c(AllR1,sum(metric.df.WT[(metric.df.WT$Fly==metric.df.WT.E1R1[i,]$Fly)&(metric.df.WT$Experimenter==metric.df.WT.E1R1[i,]$Experimenter)&!is.na(metric.df.WT$Value),]$Value[1:5]))
  }
  ...
  colnames(metric.df.WT.AllT1) = c("Value","Fly","Category","Gender","Genotype","Experimenter","Age","Setup","Session","All_Heat_Received")
  

  longer_heat_allT1 = metric.df.WT.AllT1[metric.df.WT.AllT1$All_Heat_Received>=median(metric.df.WT.AllT1$All_Heat_Received),]
  shorter_heat_allT1 = metric.df.WT.AllT1[metric.df.WT.AllT1$All_Heat_Received<median(metric.df.WT.AllT1$All_Heat_Received),]
  
  
  metric.df.WT.AllR1 = cbind(metric.df.WT.E1R1,
                             AllR1
                             )
  colnames(metric.df.WT.AllR1) = c("Value","Fly","Category","Gender","Genotype","Experimenter","Age","Setup","Session","All_Heat_Received")
  ...
  metric.df.WT.T1 = subset(metric.df.WT.T1,metric.df.WT.T1$Experimenter!="RS")
  
  metric.df.WT.R1 = rbind(longer_heat_allR1, shorter_heat_allR1)
  metric.df.WT.R1 = subset(metric.df.WT.R1,metric.df.WT.R1$Experimenter!="RS")
  ...
  query.fly <- fly.info.include[fly.info.include$Genotype=="WT",]$Fly
  ...
  sessions <- c("E1T1E1","E1R1E1","E1N1E1",
                ...
                "E1T1E1T1E1T1E1","E1R1E1R1E1R1E1","E1N1E1N1E1N1E1",
                "E1T1E1T1E1T1E1T1E1","E1R1E1R1E1R1E1R1E1","E1N1E1N1E1N1E1N1E1")
  ...
  pdf("fly_metric_allmetricdf_CS_051818_JDES_allflies_Filter1.pdf",onefile=T,width=10)
  ...
  for(metric.ind in 1:length(metrices)){
    ...
    input.yy = c(input.y_1T,input.y_1R,input.y_1N, input.y_2T,input.y_2R,input.y_2N,input.y_3T,input.y_3R,input.y_3N)
    ...
    stripchart(Value~Sessions,vertical = TRUE, data = input.y.df,method = "jitter",
  ...
  for(ind in 1:nrow(metric.df.WT.R1)){
    ...
    for(ind.session in 1:length(query.sessions)){
      input.file <- list.files(path = paste0("data/",
                                             ...
                                             "/CS/"),                             
                               pattern = paste0("ProcessedData_Fly",metric.df.WT.R1$Fly[ind],
                                                "_",query.sessions[ind.session],
                                                "_WT",
                                                ".csv"),
                               full.names=T
