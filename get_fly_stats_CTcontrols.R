...
one_fly_statistics <- function(input_file,
                               framerate = 50,
                               speed_max_thres = 30,
                               #updated from 20 to 30 on Jan 31, 2018
                               ...
                               chamber_end_thres = 50)
{
  ...
  pause_middle_dur <-
    subset(pause_df,
           (Pause_Duration >= 25) &
             (Start_Position >= 50) & (Start_Position <= 717))$Pause_Duration
  avg_pause_middle_dur <- (mean(pause_middle_dur)) / framerate
  frac_pause_middle <- (sum(pause_middle_dur)) / experiment_time
  max_pause_middle <- (max(pause_middle_dur)) / framerate
  first_pause_middle <- (pause_middle_dur[1]) / framerate
  ...
  pause_middle_nobump_df = subset(pause_df,
                                  ...
                                  & (Start_Position <= 717)
                                  &
                                    (((Start_Type == 1) & (End_Type == 2)) |
                                       ((Start_Type == 4) & (End_Type == 3))))
  pause_middle_bump_df = subset(pause_df,
                                (Start_Position >= 50)
                                & (Start_Position <= 717)
                                &
                                  (((Start_Type == 1) & (End_Type == 3)) |
                                     ((Start_Type == 4) & (End_Type == 2))))
  ...
  ret = list(
    ...
    (((tot_moving_dist) * (48.7 - 1) / 768) / experiment_time) * framerate *
      60,
    num_turn,
    #NU
    num_mid_turns,
...
fly.info = read.csv(
  "data/fly_info_CS_controls_preprocessed.csv",
  header = T,
  stringsAsFactors = F
)
