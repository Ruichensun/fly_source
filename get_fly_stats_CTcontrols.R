...
# source("get_fly_stats_laserplots.R")
...
one_fly_statistics <- function(input_file,
                               framerate = 50,
                               speed_max_thres = 30, #updated from 20 to 30 on Jan 31, 2018
...
)
{
  ...
  if (length(ends)<1){
    ...
    end_type = label_for_pause[ends]
    pause_df = data.frame(starts[1:length(ends)]-1,start_position[1:length(ends)], start_type[1:length(ends)], ends,end_position, end_type, ends-starts[1:length(ends)])
  }else{pause_df = data.frame(starts[1:length(ends)]-1,start_position[1:length(ends)], start_type[1:length(ends)], ends-1,end_position, end_type, ends-starts[1:length(ends)])}
  ...
  if(num_pause!=0){
    for(i in 1:num_pause){
      ...
      if(ps < window_size || pe > experiment_time-window_size){
        ...
        fly_speed_at_pause_start = c(fly_speed_at_pause_start, ## when framerate changes, this needs to be changed as well. currently 15 frames here means 300ms
  ...
  if (num_pause<=10){
    ...
    # burstiness_pause = (sd((pause_end-pause_start),na.rm = T)-mean((pause_end-pause_start),na.rm = T))/(sd((pause_end-pause_start),na.rm = T)+mean((pause_end-pause_start),na.rm = T))
    burstiness_pause = (sd(Pause_duration,na.rm = T)-mean(Pause_duration,na.rm = T))/(sd(Pause_duration,na.rm = T)+mean(Pause_duration,na.rm = T))
  }
  ...
  if (length(inter_event_time)<=10){
    ...
    Burst_inter_event = (sd((inter_event_time),na.rm = T)-mean((inter_event_time),na.rm = T))/(sd((inter_event_time),na.rm = T)+mean((inter_event_time),na.rm=T))
  }
  ...
  if (length(inter_event_time_scrambled)<=10){
    ...
    Burst_inter_event_scrambled = (sd((inter_event_time_scrambled),na.rm = T)-mean((inter_event_time_scrambled),na.rm = T))/(sd((inter_event_time_scrambled),na.rm = T)+mean((inter_event_time_scrambled),na.rm=T))
  }
  
  
  ######inverted burstiness_no thresholding########
  burstiness_inter_event_inverted<-rep(0,length(burstiness_inter_event))#replace(abs(fly_speed),abs(fly_speed)>0,1)
  burstiness_inter_event_inverted<-replace(burstiness_inter_event_inverted,burstiness_inter_event==0,1)
  inter_event_time_inverted<-rle(burstiness_inter_event_inverted)$length[rle(burstiness_inter_event_inverted)$values==0]
  if (length(inter_event_time_inverted)<=10){
    # Burst_inter_event = 1
    Burst_inter_event_inverted=NA
  }else{
    Burst_inter_event_inverted = (sd((inter_event_time_inverted),na.rm = T)-mean((inter_event_time_inverted),na.rm = T))/(sd((inter_event_time_inverted),na.rm = T)+mean((inter_event_time_inverted),na.rm=T))
  }
  ...
  if (num_pause<=10){
    ...
    burstiness_pause_inverted = (sd((pause_start_late - pause_end_late),na.rm = T)-mean((pause_start_late - pause_end_late),na.rm = T))/(sd((pause_start_late - pause_end_late),na.rm = T)+mean((pause_start_late - pause_end_late),na.rm = T))
  }
  
  ########Get behavioral states#######
  # burstiness_inter_event<-replace(abs(fly_speed),abs(fly_speed)>0,1)
  
  fly_pos_original = burstiness_inter_event[1:(length(burstiness_inter_event)-1)]
  ...
  if(num_pause<2){
    w_to_w = 0
  }else{w_to_w = sum(pause_df$Start_Index[2:length(pause_df$Start_Index)]-pause_df$End_Index[1:(length(pause_df$Start_Index)-1)]-1)}
  ...
  fly_pos_sum_middle = (is_pause_middle[2:(length(is_pause_middle)-1)])*1 + 2*(is_pause_middle[3:length(is_pause_middle)])+4*(is_pause_middle[1:(length(is_pause_middle)-2)])
  # 
  ...
  pause_middle_nobump_df = subset(pause_df,
                                  (Start_Position>=50)
                                  &(Start_Position<=717)
                                  &(((Start_Type==1)&(End_Type==2))|((Start_Type==4)&(End_Type==3))))
  pause_middle_bump_df = subset(pause_df,
                                (Start_Position>=50)
                                &(Start_Position<=717)
                                &(((Start_Type==1)&(End_Type==3))|((Start_Type==4)&(End_Type==2))))
  ...
  if(num_pause<2){
    ...
    if(num_pause_middle_nobump<2){
      ...
      w_to_w_middle_nobump = sum(pause_middle_nobump_df$Start_Index[2:length(pause_middle_nobump_df$Start_Index)]-pause_middle_nobump_df$End_Index[1:(length(pause_middle_nobump_df$Start_Index)-1)]-1) - sum(p_to_p_end,p_to_w_end,w_to_p_end,p_to_p_middle_bump,p_to_w_middle_bump,w_to_p_middle_bump)
    }
  }
  ...
  start_walking_middle = replace(fly_pos_sum_middle,fly_pos_sum_middle!=4, 0)
  
  start_walk_burst_middle=rle(start_walking_middle)$length[rle(start_walking_middle)$values==0]
  
  if (length(start_walk_burst_middle)<=10){
    # Burst_inter_event = 1
    Burst_start_walking_middle=NA
  }else{
    Burst_start_walking_middle = (sd((start_walk_burst_middle),na.rm = T)-mean((start_walk_burst_middle),na.rm = T))/(sd((start_walk_burst_middle),na.rm = T)+mean((start_walk_burst_middle),na.rm=T))
  }
  ...
  start_pause_burst_middle=rle(start_pause_middle)$length[rle(start_pause_middle)$values==0]
  
  if (length(start_pause_burst_middle)<=10){
    # Burst_inter_event = 1
    Burst_start_pause_middle=NA
  }else{
    Burst_start_pause_middle = (sd((start_pause_burst_middle),na.rm = T)-mean((start_pause_burst_middle),na.rm = T))/(sd((start_pause_burst_middle),na.rm = T)+mean((start_pause_burst_middle),na.rm=T))
  }
  ...
  if (length(inter_event_time_inverted)<2){
    ...
    for (i in 1:(length(inter_event_time_inverted)-1)){
      memory_inverted<-memory_inverted+((inter_event_time_inverted[i]-m3)*(inter_event_time_inverted[i+1]-m4)/(std3*std4))
    }        
  ...
  frac_pause = sum((pause_middle_nobump_df$Pause_Duration),na.rm=TRUE) / experiment_time
  ...
  frac_mid_turns = num_turn/num_mid_turns
  
  
  ## Return output
  ret = list(
    num_pause,
    frac_pause,#unit: percentage
    ...
    (((tot_moving_dist)*(48.7-1)/768)/experiment_time)*framerate*60,
    num_turn,#NU
    num_mid_turns,#NU
    frac_mid_turns,#NU
    burstiness_pause,
...
shuffle_is_pause <- function(is_pause) {
  if(max(is_pause) == 0) {return(is_pause)}
  r = rle(is_pause)
...
all_ofs_WT_controls = list()
for(session in sessions){
  all_ofs_WT_controls = append(all_ofs_WT_controls,list(c()))
}
## read fly info
...
for(ind in 1:nrow(fly.info)){
  ...
  for(ind.session in 1:length(query.sessions)){
    ...
    if(length(input.file) == 0){
      all_ofs_WT_controls[[sessions[ind.session]]] = append(all_ofs_WT_controls[[sessions[ind.session]]],list(NA))
      next
    }   
    framerate = fly.info$Framerate[ind]        
    ofs = one_fly_statistics(input.file,framerate=framerate)
    
    all_ofs_WT_controls[[sessions[ind.session]]] = append(all_ofs_WT_controls[[sessions[ind.session]]],list(ofs))
  }
}
...
#   }
# }
save.image("all_ofs_WT_controls.Rdata")
